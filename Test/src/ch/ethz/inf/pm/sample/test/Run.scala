...
object Run {
  ...
  def iteratorOverFiles[N <: SemanticDomain[N], H <: HeapDomain[H, I], I <: HeapIdentifier[I]](file: File, f: (File, File, List[String], Analysis, HeapAndAnotherDomain[N, H, I]) => Unit) {
    if (file.isDirectory) {
      if (!file.getName.equals(".svn")) {
        try {
          ...
          val entryState: HeapAndAnotherDomain[N, H, I] = r._2
          val analysis = r._3
  ...
  def initRun[I <: HeapIdentifier[I], H <: HeapDomain[H, I], N <: SemanticDomain[N]](testFile: File, sourceCodeFile: File, methods: List[String], analysis: Analysis, entryDomain: HeapAndAnotherDomain[N, H, I]): (Set[ExpectedOutput], Set[Output]) = {
    ...
    SystemParameters.setAnalysisOutput(new StdOutOutput)
    SystemParameters.setProgressOutput(new StdOutOutput)
  ...
  private def check(exp: Set[ExpectedOutput], output: Set[Output]): (Set[ExpectedOutput], Set[Output], Int) = {
    ...
    (missedExpectedOutput, notCoveredOutput, matched)
  }
}
