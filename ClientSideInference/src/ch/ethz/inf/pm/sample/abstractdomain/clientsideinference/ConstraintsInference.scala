...
import lpsolve._
import property.{OutputCollector, SingleStatementProperty, Property}
;
...
class ConstraintsInference[S <: SymbolicValue[S], T <: SymbolicInt[T, S]] {
  ...
  def solve(constraints : Set[Constraint[S]]) : Map[S, Double] = {
      try {
        System.out.println(constraints.mkString("\n"))
        val vars : List[S] = this.extractVariables(constraints).toList;
  ...
  private def extractVariables(set : Set[Constraint[S]]) : Set[S] = {
    var variables = Set.empty[S];
    for(c <- set)
      variables=variables++extractVariables(c);
    return variables
  }

  private def extractVariables(c : Constraint[S]) : Set[S] = c match {
  ...
  private def extractVariables(a : ArithmeticExpression[S]) : Set[S] = a match {
    case Add(left, right) => return extractVariables(left)++extractVariables(right)
    case SimpleVal(value) => Set.empty[S]
    case Multiply(mul, right) => Set.empty[S]+right
  }

  private def addConstraints(constraints : Set[Constraint[S]], variables : List[S], solver : LpSolve) = {
    val l = constraints;
    for(c <- constraints) {
  ...
  private def extractConstraint(constraint : Constraint[S], variables : List[S]) : (String, Double, Int) = constraint match {
  ...
  private def extractExpressions(a : ArithmeticExpression[S], vars : List[S]) : (Array[Int], Double) = a match {
    case Add(left, right) =>
    ...
    case Multiply(mul, right) =>
      ...
      result(vars.indexOf(right))=mul match {
        case SimpleVal(value) => value
      };
      return (result, 0);
  }

  //TODO: Parameterize it for different weakest/strongest precondition/postcondition
  private def addObjectiveFunction(variables : List[S], solver : LpSolve) = {
    var s : String = "";
    for(v <- variables) {
      if(v.minimize())
        s=s+"-1 ";
...
trait ConstraintsInferenceProperty[S <: SymbolicValue[S], T <: SymbolicInt[T, S]] extends Property {
  def getConstraintsInference() : ConstraintsInference[S, T];
}

class ConstraintSolverProperty[S <: SymbolicValue[S], T <: SymbolicInt[T, S]](val p : ConstraintsInferenceProperty[S, T]) extends Property {

  def getLabel() : String = "Constraint solver of "+p.getLabel();
  def finalizeChecking(printer : OutputCollector) : Unit = {
    p.finalizeChecking(printer);
    val c = p.getConstraintsInference();
    c.printConstraints()
    val result=c.solve(c.getConstraints());
    if(result==null)
      SystemParameters.analysisOutput.appendString("The system is infeasible")
    else {
      if(result.isEmpty)
        SystemParameters.analysisOutput.appendString("You do not need any contract")
    }
      for(s <- result.keySet)
        SystemParameters.analysisOutput.appendString(s.toString+" = "+result.get(s))
  }

  def check[S <: State[S]](classe : Type, methodName : String, result : ControlFlowGraphExecution[S], printer : OutputCollector) : Unit = p.check(classe, methodName, result, printer)
}
