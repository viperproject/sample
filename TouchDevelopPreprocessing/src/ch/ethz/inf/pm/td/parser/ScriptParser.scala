...
object ScriptParser extends RegexParsers with PackratParsers {
  ...
  lazy val tableContent: PackratParser[(String, Option[String], List[Parameter], List[Parameter], Boolean, Boolean, Boolean, Boolean)] = (
    ("type" ~ "=" ~ stringLiteral ~ ";" ~
      ...
      case _ ~ _ ~ a ~ _ ~ sN ~ cE ~ cPE ~ p ~ e ~ b ~ c =>
        ...
        val sourceName = cE match {
          case Some(_ ~ _ ~ x ~ _) => Some(x.toString);
          case None => None
        }
        val cloudEnabled = cE match {
          case Some(_ ~ _ ~ x ~ _) => x.toBoolean;
          case None => false
        }
        val cloudPartiallyEnabled = cPE match {
          case Some(_ ~ _ ~ x ~ _) => x.toBoolean;
          case None => false
        }
        val persistent = p match {
          case Some(_ ~ _ ~ x ~ _) => x.toBoolean;
          case None => false
        }
        val exported = e match {
          case Some(_ ~ _ ~ x ~ _) => x.toBoolean;
          case None => false
        }
  ...
  lazy val whereStmt: PackratParser[Statement] = positioned(
    (expression <~ ";") ~ rep1("where" ~> actionHeader ~ block) ^^ { case expr ~ inlineActions => WhereStatement(expr, inlineActions map { case (n, in, out) ~ b => InlineAction(n, in, out, b, inParametersToActionType(in)) }, Nil) }
  )
  ...
  lazy val propertyAccess: PackratParser[Expression] = positioned(
    propertyAccess ~ rA ~ posIdent ~ ("(" ~> repsep(orExpression, ",") <~ ")").?
      ^^ { case ex ~ _ ~ prop ~ args => args match {
      case None => Access(ex, prop, Nil);
      case Some(x) => Access(ex, prop, x)
    }
    }
      ||| terminalExpression
  )
  ...
  lazy val escapedString: Parser[String] =
  """(?:[^"\\]+|\\.)*""".r
  //lazy val escapedString:Parser[String] = """[^"\\]*(\\.[^"\\]*)*""".r
  lazy val ident: Parser[String] =
  """[a-zA-z@_◈](?:\w|\\.)*""".r ^^ (StringEscapeUtils.unescapeJava(_).replace("_", " "))
