...
object SBazaar extends ASingleton {
  ...
  lazy val field_leaderboard_score = new TouchField("leaderboard score", TNumber.typeName)

  /** Returns the user object of the current user */
  lazy val field_current_user = new TouchField("current user", TUser.typeName)

  lazy val typeName = TypeName("Bazaar")

  override def possibleFields = super.possibleFields ++ List(field_current_user, field_leaderboard_score)
