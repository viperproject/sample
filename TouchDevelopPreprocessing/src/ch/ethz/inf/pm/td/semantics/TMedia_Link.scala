...
object TMedia_Link {
  ...
  val field_album = new TouchField("album",TString.typ,TopWithInvalidInitializer())

  /** Gets the author if available */
  val field_author = new TouchField("author",TString.typ,TopWithInvalidInitializer())

  /** Gets the date if available */
  val field_date = new TouchField("date",TDateTime.typ,TopWithInvalidInitializer())

  /** Gets the duration in seconds (0 for pictures) */
  val field_duration = new TouchField("duration",TNumber.typ,TopInitializer())

  /** Gets the kind of media (video, song, picture) */
  val field_kind = new TouchField("kind",TString.typ,TopInitializer())

  /** Gets the title if available */
  val field_title = new TouchField("title",TString.typ,TopWithInvalidInitializer())
