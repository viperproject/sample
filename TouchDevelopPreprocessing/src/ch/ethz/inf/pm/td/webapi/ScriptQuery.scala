...
class ScriptQuery extends IteratorOverPrograms {
  ...
  private def getNextScripts: List[ScriptRecord] = {
    ...
    (for (
      ...
      item <- items
    ) yield { item.extract[ScriptRecord] }).filter(filter _)
...
trait DateFilter extends ScriptQuery {
  ...
  private var d:java.util.Date = new java.util.Date() // now
  def setDate(s:java.util.Date) { d = s }
...
trait PrefixFilter extends ScriptQuery {
  ...
  private var prefix:String = "" // any
  def setPrefix(s:String) { prefix = s }
