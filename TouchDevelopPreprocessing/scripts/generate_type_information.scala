...
val p = new PrintWriter(new File("TouchDevelopPreprocessing/gen/Symbols.scala"))
  ...
  for (
    ...
    val fields = (for (
      ...
      val paramList = (for (
        ...
        if (paramName != "this")
          Some("\""+WebASTImporter.convertType(paramType)+"\"")
        else
          None
      ...
      if (paramList.size > 0) {
        additional ::: List("    Member(\""+propName+"\", List("+paramList.mkString(", ")+"), \""+WebASTImporter.convertType(resultTyp)+"\") /* "+propHelp+" */")
      } else {
        additional ::: List("    Member(\""+propName+"\", \""+WebASTImporter.convertType(resultTyp)+"\") /* "+propHelp+" */")
      }
    ...
    p.println("  // "+help)
    if(!isData) {
      p.println("  addSingleton(\""+name+"\", gAlsoSingletons(\""+name+"\") ::: List(")
    } else {
      p.println("  addType(\""+name+"\", gAny(\""+name+"\") ::: List(")
    }
