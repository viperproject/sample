...
try {
  ...
  for (
    ...
    val fields = (for (
      ...
      if (paramList.size > 0) {
        additional ::: List("    Member(\""+propName+"\",List("+paramList.mkString(",")+"),\""+resultTyp+"\") /* "+propHelp+" */")
      } else {
        additional ::: List("    Member(\""+propName+"\",\""+resultTyp+"\") /* "+propHelp+" */")
      }
    ...
    p.println("  // "+help)
    if(!isData) {
      p.println("  addSingleton(\""+name+"\", List( ")
    } else {
      p.println("  addType(\""+name+"\", List( ")
    }
