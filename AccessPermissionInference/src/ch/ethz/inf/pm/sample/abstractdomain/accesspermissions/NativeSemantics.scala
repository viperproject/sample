            ...
            case "share" =>
              for (exp <- ids)
                exp match {
                  ...
                  case id: MayHeapSetDomain[ProgramPointHeapIdentifier] => result = Annotation.exhaleInvariants(exp.asInstanceOf[MayHeapSetDomain[ProgramPointHeapIdentifier]], exp.typ.name, castedState)
                }
            case "unshare" =>
            /*if(ids.size == 1)
              for(exp <- ids)
              ...
              if (ids.size == 1)
                for (exp <- ids)
                  exp match {
                    ...
                    case id: MayHeapSetDomain[ProgramPointHeapIdentifier] => result = Annotation.inhaleInvariants(id, exp.typ.name, castedState)
                  }
            case "release" =>
              for (exp <- ids)
                exp match {
                  case id: Identifier => result = Annotation.exhaleInvariants(id, exp.typ.name, castedState)
                  case id: MayHeapSetDomain[ProgramPointHeapIdentifier] => result = Annotation.exhaleInvariants(id, exp.typ.name, castedState)
                }
            case "free" =>
              for (exp <- ids)
                exp match {
                  case id: Identifier => result = Annotation.exhaleEverything(id, castedState)
                  case id: MayHeapSetDomain[ProgramPointHeapIdentifier] => result = Annotation.exhaleEverything(id, castedState)
                }
          }
                ...
                if (ids.size == 1)
                  for (exp <- ids)
                    exp match {
                      ...
                      case id: MayHeapSetDomain[ProgramPointHeapIdentifier] => result = Annotation.inhalePredicate(id, s, castedState)
                    }
              case "unfold" =>
                for (exp <- ids)
                  exp match {
                  ...
                  case Constant(s, typ, pp) =>
                    ...
                    for (exp <- x.getSetOfExpressions)
                      exp match {
                        ...
                        case id: MayHeapSetDomain[ProgramPointHeapIdentifier] => result = Annotation.exhalePrecondition(id, x.getType().toString, s, castedState, result);
                      }
                  ...
                  case Constant(s, typ, pp) =>
                    ...
                    for (exp <- x.getSetOfExpressions)
                      exp match {
                        ...
                        case id: MayHeapSetDomain[ProgramPointHeapIdentifier] => Annotation.inhalePostcondition(id, x.getType().toString, s, castedState, result);
                      }
                    val d1 = HeapAndAnotherDomain[P, NonRelationalHeapDomain[ProgramPointHeapIdentifier], ProgramPointHeapIdentifier](result, castedState._1._2);
                    val entryvalue = thisExpr.top().asInstanceOf[ExpressionSet]
                    return new Some(new AbstractState[P, NonRelationalHeapDomain[ProgramPointHeapIdentifier], ProgramPointHeapIdentifier](d1, entryvalue).asInstanceOf[S])
    ...
    case className => {
      ...
      for (exp <- thisExpr.getSetOfExpressions)
        exp match {
          ...
          case id: MayHeapSetDomain[ProgramPointHeapIdentifier] => result = Annotation.exhalePrecondition(id, className, operator, castedState, result);
        }
      for (exp <- thisExpr.getSetOfExpressions)
        exp match {
          case id: Identifier => result = Annotation.inhalePostcondition(id, className, operator, castedState, result);
          case id: MayHeapSetDomain[ProgramPointHeapIdentifier] => result = Annotation.inhalePostcondition(id, className, operator, castedState, result);
        }
      val d1 = HeapAndAnotherDomain[P, NonRelationalHeapDomain[ProgramPointHeapIdentifier], ProgramPointHeapIdentifier](result, castedState._1._2);
