					 ...
					 bool div2)
{
  ...
  if (!isfinite(d)) ap_scalar_set_infty(r,1);
   ...
   /* case AP_SCALAR_MPQ:
      if (!mpq_set_num(r->val.mpq,d) || div2) pr->conv = 1;
      if (div2) mpq_div_2exp(r->val.mpq,r->val.mpq,1);
      break;
    case AP_SCALAR_MPFR:
      if (!mpfr_set_num(r->val.mpfr,d) || div2) pr->conv = 1;
...
bool opt_oct_sat_lincons(ap_manager_t* man, opt_oct_t* o, ap_lincons0_t* lincons);
