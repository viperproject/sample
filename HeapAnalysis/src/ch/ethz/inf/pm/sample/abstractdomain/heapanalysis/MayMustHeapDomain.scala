...
//import ch.ethz.inf.pm.sample.oorepresentation.{NativeMethodSemantics, Type, ProgramPoint}
...
//
//  def lub(l: T, r: T): T = factory(may.lub(l.may, r.may),must.glb(l.must, r.must))
//
//  def glb(l: T, r: T): T = factory(may.glb(l.may, r.may),must.glb(l.must, r.must))
//
//  def widening(l: T, r: T): T = factory(may.widening(l.may, r.may),must.glb(l.must, r.must))
//
...
//
//  def getIds() = may.getIds() ++ must.getIds()
//
//  def setToTop(variable: Identifier): T = factory(may.setToTop(variable),must.setToBottom(variable))
//
...
//
//  def backwardAssign(variable: Identifier, expr: Expression): T = factory(may.backwardAssign(variable,expr),must.backwardAssign(variable,expr))
//
...
//
//
//class MayMustHeapEnvironment extends FunctionalDomain[Identifier,HeapSet,MayMustHeapEnvironment] {
//
//  /**
...
//   */
//  def get(key: Identifier): MaybeHeapIdSetDomain[MayMustIdentifier] = this.value.get(key) match {
//    case None => dom.top()
//    case Some(x) => x
//  }
//
...
//   */
//  def factory(): MayMustHeapEnvironment = new MayMustHeapEnvironment
//
//}
//
//class HeapSet extends SetDomain[MayMustIdentifier,HeapSet] {
//
//  /**
//   * Returns a new instance of the lattice
//   *
//   * @return A new instance of the current object
//   */
//  def factory(): HeapSet = new HeapSet
//
//}
//
//class MayMustIdentifier (pp:ProgramPoint,typ:Type,summary:Boolean,count:Integer) extends HeapIdentifier[MayMustIdentifier] {
//
//  /**
...
//}
//
//class FieldAndMayMustIdentifier (pp:MayMustIdentifier,name:String,typ:Type) extends HeapIdentifier[FieldAndMayMustIdentifier] {
//
//  /**
//  Returns the name of the identifier. We suppose that if two identifiers return the same name if and only
//   if they are the same identifier
...
//   @return true iff this identifier represents exactly one variable
//    */
//  def representSingleVariable(): Boolean = pp.representSingleVariable()
//
//}