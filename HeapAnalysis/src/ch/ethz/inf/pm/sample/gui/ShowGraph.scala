package ch.ethz.inf.pm.sample.gui
...
object ShowGraph extends Property
{
    ...
    def ShowControlFlowGraph(graph : ControlFlowGraph) : Unit = new Show(ShowGraph.ControlFlowGraphJGraph(graph), true, -1, -1);
	...
	private class ShowControlFlowGraphExecution[S <: State[S]] {
	  def this(g : ControlFlowGraphExecution[S]) = {
	    ...
	    new Show(graphComponent, true, -1, -1);
	  }
	}
