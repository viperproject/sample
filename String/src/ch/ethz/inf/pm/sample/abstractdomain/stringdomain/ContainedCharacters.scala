...
case class InverseSetCharacters(wrapped: SetDomain.Default[Char] = SetDomain.Default.Bottom[Char]())
  extends InvertedLatticeWrapper[SetDomain.Default[Char], InverseSetCharacters] {

  override def wrapperFactory(wrapped: Default[Char]): InverseSetCharacters = InverseSetCharacters(wrapped)
