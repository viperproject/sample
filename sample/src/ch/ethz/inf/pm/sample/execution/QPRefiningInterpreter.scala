...
/**
  ...
  *         Added on 21/10/16.
  */
trait QPRefiningInterpreter[S <: State[S]] extends Interpreter[S] with LazyLogging {
  ...
  /**
    ...
    */
  def preprocessingGraph(cfg: ControlFlowGraph): (ControlFlowGraph, mutable.LinkedHashSet[Int]) = {
  ...
  def traverseGraphDFSFindLoop(currentNode: Int, cfg: ControlFlowGraph, path: Set[Int]): (Set[Int], mutable.LinkedHashSet[Int]) = {
  ...
  def refiningExecute(p_cfg: ControlFlowGraph, finalState: S, forwardStates: C): C = {
    val (cfg, blocksToProcessIds) = preprocessingGraph(p_cfg)
    val cfgState = forwardStates //cfgStateFactory.allBottom(cfg) // start with the forwardStates
    ...
    while (blocksToProcessIds.nonEmpty) {
      ...
      val postState: S = if (leavesIds.contains(currentId)) cfgState.statesOfBlock(currentId).last
        ...
        exitEdges.size match {
          ...
          case 1 => {
            //Remove the expression from the oldPreState, there might be some expressions left and backwardLub would transform it into a branch cond
  ...
  private def refiningExecuteBlock(exitState: S, id: Int, cfgState: C, forwardStates: C): Unit = {
    ...
    for ((stmt: Statement, idx: Int) <- stmts.zipWithIndex.reverse) {
      ...
      if (whileLoopCond.contains(id) && idx == 0) {
        ...
        val (_, to2, weight2) = set.last
        val (loopBodyNr, afterLoopNr) = if (weight1 == Some(true)) (to1, to2) else (to2, to1)
        val (loopBodyFirstState, loopBodyLastState, afterLoopState) = (cfgState.statesOfBlock(loopBodyNr).head, cfgState.statesOfBlock(loopBodyNr).last, cfgState.statesOfBlock(afterLoopNr).head)
        val beforeLoopState = exitState
...
/** Backward Interpreter over TrackingCFGState.
  ...
  */
case class TrackingQPRefiningInterpreter[S <: State[S]](stateFactory: S) extends QPRefiningInterpreter[S] {
