...
/** @note Store the path as a list of identifiers rather than strings, so that
  * no information (identifier structure, types, program points) are lost,
  * when building the `AccessPathIdentifier`.
 */
